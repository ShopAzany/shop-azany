...

{
    ...
    public static function singleProductForSeller($sellerID, $pid){
      ...
      return self::where(function($p) use($pid, $sellerID){
            $p->where('pid', '=', $pid);
            $p->where('seller_id', '=', $sellerID);
            $p->orWhere('admin_id', '=', $sellerID);
       })->first();//query roomNumber model table
    ...
    public static function sellerProduct($pageLimit, $offset, $sellerID){
        ...
        return self::take($pageLimit)->offset($offset)->whereRaw("seller_id = '$sellerID' OR admin_id = '$sellerID' AND status != 'Removed'")->orderBy('pid', 'DESC')->get();
    ...
    public static function sellerProductCount($sellerID){
        ...
        return self::whereRaw("seller_id = '$sellerID' OR admin_id = '$sellerID' AND status != 'Removed'")->count();
    ...
    public static function productBySeller($sellerID){
        ...
        return self::where(function($p) use($sellerID){
            $p->where('seller_id', '=', $sellerID);
            $p->orWhere('admin_id', '=', $sellerID);
       })->orderBy('pid', 'DESC')->get();//query roomNumber model table
    ...
    public static function createRow($sellerID, $data){
        ...
        if ($sellerID && $data->merchant_type && $data->category && $data->title && $data->description) {
    ...
    public static function createNewRecord($data){

        if ($data->seller_id && $data->merchant_type && $data->category && $data->title && $data->description) {
    ...
    public static function lastRow($sellerID){
        ...
        return self::where(function($p) use($sellerID){
            $p->where('seller_id', '=', $sellerID);
            $p->orWhere('admin_id', '=', $sellerID);
       })->orderBy('pid', 'DESC')->first();//query table
    ...
    public static function otherSellerProduct($sellerID, $pid, $loginID){
        ...
        $sellerProFour = self::where('seller_id', $sellerID)->orWhere('admin_id', $sellerID)->where('status', 'active')->where('pid', '!=', $pid)->take(4)->get();
    ...
    public static function adminCreateProduct($data=array()){

        if ($data['admin_id'] && $data['title'] && $data['category']) {
