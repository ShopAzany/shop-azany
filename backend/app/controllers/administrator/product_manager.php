<?php
	...
	*/
	...
	class product_manager extends controller{
       ...
       public function admin_add_product_page(){
        $this->view('administrator/admin-add-product-page', [
       ...
       public function add_admin_product(){
			...
			$adminUp = Products::adminCreateProduct($items);

			if ($adminUp) {

				Redirect::to(Config::ADMIN_BASE_URL().'product_manager/edit_image/'.$adminUp['pid']);
		...
		public function update_edited_pro_pricing(){
			...
			foreach (Input::all()['theVariation'] as $value) {
				...
				$percent = 100;
					...
					$regular = $value['regular_price'];
					$sale = $value['sales_price'];
					$percent_divide = $regular / $percent;

					//$sum = $regular - $sale * $percent_divide;
                    $sum = ($regular - $sale) / $percent_divide;
