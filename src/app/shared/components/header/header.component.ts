...
import { Component, Input, OnInit, ElementRef, ViewChildren, QueryList, AfterViewInit, ViewEncapsulation, Inject, PLATFORM_ID } from '@angular/core';
...
export class HeaderComponent implements OnInit, AfterViewInit {
  ...
  //           { subsubcat_name: 'Shirt', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Shoes', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Shorts', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Trousers', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Suits', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Sneakers', subsubcat_slug: '/test' },
  //         ],
  //       },
  //       {
  ...
  //           { subsubcat_name: 'Shirt', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Shoes', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Skirts', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Gowns', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Jump Suits', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Sneakers', subsubcat_slug: '/test' },
  //         ],
  //       },
  //       {
  ...
  //           { subsubcat_name: 'Shorts', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Trousers', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Round necks', subsubcat_slug: '/test' },
  //         ],
  //       },
  //       {
  //         subcat_name: 'Baby',
  ...
  //           { subsubcat_name: 'Shoes', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Shorts', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Gloves', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Hats', subsubcat_slug: '/test' },
  //         ],
  //       },
  //       {
  ...
  //         subcat_slug: '/test',
  //         subcat_subcategories: [
  //           { subsubcat_name: 'Sun Glasses', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Watches', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Scarfs', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Hats', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'jewery', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Socks', subsubcat_slug: '/test' },
  //         ],
  //       },
  //       {
  //         subcat_name: 'Brands',
  //         subcat_slug: '/test',
  //         subcat_subcategories: [
  //           { subsubcat_name: 'Nike', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Addidas', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Puma', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Sketchers', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Zara', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'New Balanace', subsubcat_slug: '/test' },
  //           { subsubcat_name: 'Balaciaga', subsubcat_slug: '/test' },
  //         ],
  //       },
  //     ]
  //   },
  ...
  @ViewChildren('dropItem') dropItem: QueryList<ElementRef>;
  @ViewChildren('clickable') clickables: QueryList<ElementRef>;
  @ViewChildren('backBtn') backBtns: QueryList<ElementRef>;

  megaMenuDropped = false;
  countryIso = 'us';
  ...
  checkSearch(e) {
    if (this.keyword.length > 1 && e.key == 'Enter') {
      this.categoryService.setSearchKeyword({ category: this.selectedCat.cat_slug, keyword: this.keyword });
      let urlValidKeyword = encodeURIComponent(this.keyword);
      this.router.navigateByUrl(`/search/${urlValidKeyword}`);
    }
  }
  ...
  private getCart() {
    this.shoppingCart.getShoppingCart.subscribe(res => {
      if (res) {
        this.noOfCartItems = res.items.length;
      }
    })
  }
  ...
  private checkRouteChange() {
    this.route.params.subscribe(param => {
      if (JSON.stringify(this.initParam) != JSON.stringify(param)) {
        this.megaMenuDropped = false;
  ...
  private getSearchKeyword() {
    let searchObj = this.categoryService.getSearchObj.value;
    if (searchObj) {
      this.selectedCat.cat_slug = searchObj.category;
      this.categories.forEach(cat => {
        if (cat.mainCats.cat_slug == searchObj.category) {
          this.selectedCat.cat_name = cat.mainCats.cat_name;
        }
      });
    }
  ...
  setCategory(name, slug) {
    this.selectedCat = { cat_name: name, cat_slug: slug };
  }
  ...
  toggleMegaMenu() {
    this.megaMenuDropped = !this.megaMenuDropped;
    document.body.scrollTop = 0;
    document.body.style.overflowY = this.megaMenuDropped ? "hidden" : "";
  }
  ...
  ngAfterViewInit() {
    this.initiateFocusEv();
    // this.initiateCountryEv();
    this.initiateSubCatEv();
  }

  private initiateFocusEv() {
    const me = this;
    function auxClick(e) {
      const path = e.path || e.composedPath();
      for (let i = 0; i < path.length - 2; i++) {
        if (path[i].classList.contains('topSearch')) {
          return;
        }
      }
      me.inputFocused = false;
      document.onclick = null;
    }
    this.inputFocus.forEach((input) => {
  ...
  setCountry(ctry) {
    this.countryIso = ctry.iso;
  }

  // private initiateCountryEv() {
  //   this.dropItem.forEach((item) => {
  //     item.nativeElement.addEventListener('click', () => {
  //       let imgUrl = item.nativeElement.children[0].firstElementChild.src;
  //       let clickable = item.nativeElement.parentElement.previousElementSibling.firstElementChild.firstElementChild;
  //       clickable.src = imgUrl;
  //     });
  //   });
  // }

  private initiateSubCatEv() {
    this.clickables.forEach((clickable) => {
      clickable.nativeElement.onclick = (e) => {
        this.openSubCat(e, clickable.nativeElement);
      }
    });
    this.backBtns.forEach((backBtn) => {
      backBtn.nativeElement.onclick = () => {
        this.backFunc(backBtn.nativeElement);
      }
    })
  }

  private backFunc(backbtn) {
    let toExitParent = backbtn.parentElement.parentElement;
    if (toExitParent.classList.contains('eachSubCatPar')) {
      toExitParent = toExitParent.parentElement;
    }
    if (toExitParent.classList.contains('eachSubSubCatPar')) {
      toExitParent.style.transform = "translateX(-100%)";
      toExitParent.previousElementSibling.parentElement.parentElement.style.overflowY = "";
      setTimeout(() => {
        toExitParent.classList.remove('cusActive');
        toExitParent.style.transform = "";
      }, 300);
      return;
    }
    toExitParent.classList.remove('cusActive');
  ...
  private openSubCat(e, link) {
    if (window.innerWidth > 650) return;
    e.stopPropagation();
    e.preventDefault();
    let realLink = link;
    if (link.classList.contains('catLink')) {
      realLink = link.parentElement;
    }
    if (realLink.nextElementSibling.classList.contains('eachSubSubCatPar')) {
      realLink.parentElement.parentElement.scrollTop = 0;
      realLink.parentElement.parentElement.style.overflowY = "hidden";
    }
    realLink.nextElementSibling.classList.add('cusActive');
  }
